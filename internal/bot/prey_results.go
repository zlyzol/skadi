...
func (pr *preyResults) printEndLog() float64 {
	...
	resultInRune := endInRune.BaseAmount.Add(endInRune.QuoteAmount).ToFloat() - startInRune.BaseAmount.Add(startInRune.QuoteAmount).ToFloat()
	...
	bigPlus100 := float64(atomic.AddInt64(&global_bigPlus, int64(resultInRune * 100)))
	pr.logger.Info().
		Str("plus", fmt.Sprintf("%.4f", resultInRune)).
	...
	pr.logger.Info().Msgf("detail prey processing result: %s/%s (%s/%s): %+v", pr.exAcc.GetName(), pr.poolAcc.GetName(), pr.base.Ticker, pr.quote.Ticker, pr.debug)
	pr.logger.Info().Msgf("PREY PROCESSING END %.4f / GLB: %.4f", resultInRune, bigPlus100 / 100)
	pr.logger.Info().Msg("****************************************")
	...
	if err == nil {
		...
		if resultInRune == 0 {
			...
			s = fmt.Sprintf("%s (%s/%s) (GLB: %.4f): plus %.4f, %s -> %s (%.4f), %s -> %s (%.4f)\n", nows, pr.base.Ticker, pr.quote.Ticker, bigPlus100 / 100, resultInRune, pr.start.BaseAmount, pr.end.BaseAmount, baseDiff, pr.start.QuoteAmount, pr.end.QuoteAmount, quoteDiff)
		}
		f.WriteString(s)
		f.Close()
	}
